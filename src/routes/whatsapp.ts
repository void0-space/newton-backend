...
const whatsappRoutes: FastifyPluginAsync = async fastify => {
  ...
  // Internal route for admin panel - require session auth and rate limit
  fastify.post('/internal/send', { preHandler: [sessionAuthMiddleware, fastify.rateLimit] }, internalSendMessage);

  // Public API route - require API key and rate limit
  fastify.post('/send', { preHandler: [apiKeyAuthMiddleware, fastify.rateLimit] }, sendMessage);

  // Job status endpoint - require API key
  fastify.get('/messages/:jobId/status', { preHandler: apiKeyAuthMiddleware }, getMessageJobStatus);

  // GET endpoint for sending messages via URL parameters - add rate limit
  fastify.get('/send', { preHandler: fastify.rateLimit }, async (request: FastifyRequest, reply) => {
    try {
